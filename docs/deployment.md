...
|---|---|---|---|---|
| `tracker-prod` (단일) | **t3.xlarge 16GB** | redis + tor + crawler + detection + api + dashboard + caddy | 워킹셋 1.7~3.3GB / 16GB (~21%) | 22 (SSH), 80/443 (Caddy TLS), 8080 (api). redis 6379는 docker network 내부 only |
...
.github/workflows/deploy.yml
  ...
  3. deploy   (concurrency: deploy-prod / serial — environment: production은 §6.2 결정으로 미사용)
        ...
        ▼ healthcheck 폴링 180초 (7 컨테이너: redis tor crawler detection api dashboard caddy)
        │
   ┌────┴────┐
...
# anti-bot 차단은 reason 기록 후 통과. retry/cooldown은 운영 기본 off.
