...
**시크릿 3종**: `OPENAI_API_KEY`, `DB_PASSWORD`, `REDIS_PASSWORD`
...
3. **단일 EC2 t3.xlarge** (Ubuntu 24.04 + Docker compose 7 컨테이너 합반)
...
- 검증·안전 가드 (이미 `deploy.yml`에 구현됨):
  ...
  3. **Container healthcheck 폴링**: Dockerfile `HEALTHCHECK` 기반 + 180s deadline 동안 7개 서비스 healthy 확인, 실패 시 자동 롤백 + 롤백 후 재검증 ([deploy.yml:273-349](../../.github/workflows/deploy.yml#L273-L349))
...
- **`.env.example`을 git에 commit할지 여부** ~~(deferred)~~ → **결정됨**: `.gitignore` 추가로 git 추적 제외. 아키텍처 임계값·우회 전략·파이프라인 구조가 공개 저장소에 노출되는 정보 유출 위험 판단 (2026-06-14)
