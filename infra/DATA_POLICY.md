...
- **시크릿 파일** (EC2 `/opt/app/secrets/*`): EBS gp3 자체가 region default `aws_ebs_encryption_by_default`에 의존(at-rest 암호화). Docker `secrets:` 키워드가 컨테이너 내부에서는 tmpfs read-only mount로 노출 → `docker inspect`·`/proc/<pid>/environ`에 평문 미노출. ADR 0001 참조
...
- 모든 애플리케이션 시크릿은 EC2 `/opt/app/secrets/<name>` 파일(chmod 600, owner root:root)에 운영자가 SSH로 직접 작성. Docker `secrets:` 키워드가 컨테이너 내부 `/run/secrets/<name>` tmpfs read-only mount로 노출하고, `infra/docker-secret-shim.sh`가 대문자 env로 변환한다.
...
| 시크릿 관리 | Secrets Manager + KMS CMK | Docker `secrets:` + EC2 디스크 파일 (ADR 0001) | **중간** — `docker inspect`/`/proc` 노출은 막지만 EC2 노드 compromise 시 전 시크릿 노출. fail2ban + ed25519 + `.pem` 1Password 백업으로 진입점 좁힘 |
