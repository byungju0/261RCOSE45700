...
## Deferred from: code review of story-3.9 (2026-06-11)

- **AC#1/#2/#6 실측 결과 부재 (운영자 핸드오프)** — `docs/ab-comparison-3-9.md` §2 정량표(agreement/Tier Recall·Precision/confusion), 비용 백분위(평균·p95·이미지 분리·escalation율), `docs/integration-smoke-3-9.md` T1 알림 E2E 라이브 캡처가 모두 플레이스홀더 상태. V9(human_label)+V10(agent_runs) 적용 PG + 실 OPENAI_API_KEY 필요. 로컬 dev DB가 수동 V5 drift라 Claude 마이그레이션 직접 적용 차단 → 운영자 `!`로 V6~V10 적용 후 `ab_compare.py --limit N` 및 알림 E2E 실행. 판정 기준(Recall T1≥0.85·정탐율≥90%·평균≤$0.005·p95≤$0.02)은 §2 체크리스트. 측정·검증 스토리 설계상 의도된 핸드오프(코드 결함 아님).
- **`load_agent_scenarios` malformed JSON 미가드** — `detection/src/mocks/llm_mock.py:31-34`. `json.load` 예외(JSONDecodeError/FileNotFoundError) 무가드 전파, fixture 루트가 list가 아닌 dict일 경우 `select_scenario`가 dict 키를 순회해 `str.get` → AttributeError. 리포 통제 fixture라 현 영향 0, `isinstance(data, list)` 가드로 명확한 오류 메시지 부여 시 하드닝 가능.
